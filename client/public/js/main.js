jQuery(document).ready(function ($) {
	...
	var MQ = deviceType(),
		bindToggle = false;
	bindEvents(MQ, true);
	// setInterval(bindEvents(MQ, true)); 
	// $(window).on('click', bindEvents(MQ, true));
	$(window).on('resize', function () { //윈도우 크기 변환 할 때 일어나는 작업
		MQ = deviceType();
		// console.log("리사이징 할떄 일어나는지 테스트 중입니다.");
	...
	function bindEvents(MQ, bool) {
		// $(window).on("mousewheel", function (event) { console.log("작동되나?"); });
		if (MQ == 'desktop' && bool) {  //데스크 탑일 경우
			...
			if (hijacking == 'on') {
				...
				// $(window).on("mousewheel", function (event) { console.log(event.originalEvent.wheelDelta, "마우스 휠 테스트, ", sectionsAvailable); });
	...
	function initHijacking() {
		...
		var visibleSection = sectionsAvailable.filter('.visible'), //visible 찾는다
			...
			// $(window).on("mousewheel", function (event) { console.log(visibleSection, "하이재킹 이닛 끝", sectionsAvailable); });
			// $(window).on("mousewheel", function (event) { console.log("두번쨰", visibleSection); });
			// $(window).on("mousewheel", function (event) { console.log("탑", topSection); });
			// $(window).on("mousewheel", function (event) { console.log("봇", bottomSection); });
	...
	function scrollHijacking(event) {
		...
		if (event.originalEvent.detail < 0 || event.originalEvent.wheelDelta > 0) {
			...
			delta--;
			if(Math.abs(delta) >= scrollThreshold) prevSection();
		} else {
			delta++;
			if(delta >= scrollThreshold) nextSection();
		}
		
		console.log(delta, "델타 값")
		return false;
	}
	...
	function prevSection(event) {
		...
		console.log(visibleSection, "PREV SECTION");
		console.log(sectionsAvailable,"PREV AVAILABLE")
	...
	function nextSection(event) {
		...
		console.log(visibleSection, "NEXT SECTION");
		console.log(sectionsAvailable,"NEXT AVAILABLE")
